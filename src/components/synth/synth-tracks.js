import { useSynthStore } from "../../stores/use-synth-store.js";
import GeneralVolumeSlider from "../volume-sliders/general-volume-slider.js";

export default function SynthTrack({
    ...
    const resetTrack = useSynthStore((state) => state.resetTrack);
        ...
        <div style={{ display: "flex", gap: "1rem"}}>
            <div className="track-settings">
                ...
                <div
                    ...
                    onClick={() => updateSynth(name, { play: !play })}
                >
                    {play ? <i className="fa-solid fa-volume-high" /> : <i className="fa-solid fa-volume-xmark" /> }
                </div>

                {/* volume slider */}
                <GeneralVolumeSlider name={name} gain={gain} update={updateSynth} />
