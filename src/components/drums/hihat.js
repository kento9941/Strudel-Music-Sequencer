...
export default function Hihat({hhStruct, setHhStruct, steps}) {
    ...
    function toggleNote(index) {
        setStruct(prev => {
            const newStr = [...prev];
            newStr[index] = prev[index] === "hh" ? "~" : "hh";
            return newStr;
        });
    }    
    // update bass struct when toggleNote is called
    useEffect(() => {
        setHhStruct(struct.join(" "));
    }, [struct]);
    ...
    useEffect(() => {
        ...
        // existing notes
        rects.attr("fill", d => (d.note === "hh" ? "white" : "#171717ff"));
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("fill", d => (d.note === "hh" ? "white" : "#171717ff"))
